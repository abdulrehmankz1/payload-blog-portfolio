        ...
        {projects.map((project: Project) => {
          ...
          return (
            <Link key={project.id} href={`/projects/${project.slug}`} style={{ textDecoration: 'none', color: 'inherit' }}>
              <article style={{ border: '1px solid #e5e7eb', borderRadius: '12px', overflow: 'hidden' }}>
                {imageUrl && (
                  <div style={{ position: 'relative', height: '200px' }}>
                    <Image src={imageUrl} alt={cover?.alt || project.title} fill style={{ objectFit: 'cover' }} />
                  </div>
                )}
                <div style={{ padding: '1.5rem' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
                    <h2 style={{ fontSize: '1.2rem', fontWeight: 600 }}>{project.title}</h2>
                    {project.year && <span style={{ color: '#999', fontSize: '0.85rem' }}>{project.year}</span>}
                  </div>
                  ...
                  <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', marginBottom: '1rem' }}>
                    {project.techStack?.slice(0, 4).map((tech) => (
                      <span key={tech.id} style={{ background: '#f1f5f9', padding: '0.2rem 0.6rem', borderRadius: '6px', fontSize: '0.75rem', color: '#475569' }}>
                        {tech.icon} {tech.name}
                      </span>
                    ))}
                  </div>
                  <div style={{ display: 'flex', gap: '0.75rem' }}>
                    {project.links?.live && <span style={{ color: '#2563eb', fontSize: '0.85rem' }}>Live Demo →</span>}
                    {project.links?.github && <span style={{ color: '#374151', fontSize: '0.85rem' }}>GitHub →</span>}
                  </div>
                </div>
              </article>
            </Link>
